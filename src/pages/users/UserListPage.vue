<template>
    <div>
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="heading">{{ t('users.title') }}</h1>
                <p class="subheading mt-1">{{ t('header.manageTeam') }}</p>
            </div>
        ...
        <Modal v-model="showModal" :title="editing ? t('users.editTitle') : t('users.inviteTitle')">
            <form @submit.prevent="handleSubmit" class="space-y-4">
                <FormField name="name" :label="t('users.formName')" v-model="form.name" :error="errors.name" />
                <FormField name="email" :label="t('auth.email')" type="email" v-model="form.email" :error="errors.email"
                    :disabled="!!editing" />
                <FormField v-if="!editing" name="password" :label="t('auth.password')" type="password" v-model="form.password"
                    :error="errors.password" />
                <FormField name="role" :label="t('users.formRole')" type="select" v-model="form.role" :options="roleOptions" />
...
const columns = computed(() => [{ key: 'name', label: t('users.formName') }, { key: 'email', label: t('auth.email') }, { key: 'role', label: t('users.formRole') }, { key: 'isActive', label: t('common.status') }]);
const roleOptions = computed(() => [{ value: 'owner', label: t('users.roleOwner') }, { value: 'manager', label: t('users.roleManager') }, { value: 'employee', label: t('users.roleEmployee') }]);
...
async function handleSubmit() {
    ...
    let hasError = false;
    if (!form.name) { errors.name = t('users.validationRequired'); hasError = true; }
    if (!form.email) { errors.email = t('users.validationRequired'); hasError = true; }
    if (!editing.value && (!form.password || form.password.length < 8)) { errors.password = t('users.validationMinChars'); hasError = true; }
    if (hasError) return;
    saving.value = true;
    try {
        if (editing.value) {
...
function confirmDelete(id: string) { if (confirm(t('users.deleteConfirm'))) { userService.remove(id).then(() => { users.value = users.value.filter((u) => u.id !== id); }); } }

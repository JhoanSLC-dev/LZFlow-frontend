<template>
    <div v-if="sale">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="heading">Sale Details</h1>
                <p class="subheading mt-1">Transaction information</p>
            </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Customer</p>
                <p class="text-base font-semibold text-dark-50">{{ sale.customerName }}</p>
            </div>
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Total Amount</p>
                <p class="text-base font-semibold text-dark-50">${{ Number(sale.total).toLocaleString() }}</p>
            </div>
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Status</p>
                <span :class="sale.status === 'completed' ? 'badge-success' : 'badge-warning'" class="mt-1">{{
                    sale.status }}</span>
            </div>
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Date</p>
                <p class="text-base font-semibold text-dark-50">{{ formatDate(sale.createdAt) }}</p>
            </div>
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Created By</p>
                <p class="text-base font-semibold text-dark-50">{{ sale.createdBy?.name || 'N/A' }}</p>
            </div>
            <div class="card p-5">
                <p class="text-xs font-medium text-dark-400 uppercase tracking-wider mb-1">Tax</p>
        ...
        <div class="card">
            <div class="card-header">
                <h3 class="text-base font-semibold text-dark-50">Items</h3>
            </div>
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-dark-700/30">
                    <thead class="bg-dark-850/50">
                        <tr>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-dark-400 uppercase">Product</th>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-dark-400 uppercase">SKU</th>
                            <th class="px-6 py-3.5 text-right text-xs font-semibold text-dark-400 uppercase">Qty</th>
                            <th class="px-6 py-3.5 text-right text-xs font-semibold text-dark-400 uppercase">Unit Price
                            </th>
                            <th class="px-6 py-3.5 text-right text-xs font-semibold text-dark-400 uppercase">Subtotal
                            </th>
                        </tr>
                    </thead>
                    ...
                    <tfoot class="bg-dark-850/30">
                        <tr>
                            <td colspan="4" class="px-6 py-3.5 text-sm font-medium text-dark-400 text-right">Subtotal
                            </td>
                            <td class="px-6 py-3.5 text-sm font-medium text-dark-200 text-right">${{
                                Number(sale.subtotal).toFixed(2) }}</td>
                        </tr>
                        <tr>
                            <td colspan="4" class="px-6 py-3.5 text-sm text-dark-400 text-right">Tax</td>
                            <td class="px-6 py-3.5 text-sm text-dark-400 text-right">${{ Number(sale.tax).toFixed(2) }}
                            </td>
                        </tr>
                        <tr>
                            <td colspan="4" class="px-6 py-3.5 text-sm font-bold text-dark-50 text-right">Total</td>
